...
function partOne(input) {
  return input.split(",").reduce((acc, string) => acc + holidayAsciiStringHelperAlgorithmAppendix1A(string), 0);
}
...
function holidayAsciiStringHelperAlgorithmAppendix1A(s) {
  return s.split("").reduce((curr, c) => ((curr + c.charCodeAt(0)) * 17) % 256, 0);
}
